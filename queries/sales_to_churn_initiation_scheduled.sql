-- Sales to Churn Initiation — Daily Scheduled Run
-- Used by GitHub Actions. Replaces Metabase {{variables}} with a rolling 90-day window.
-- Results are written to the destination table configured in the workflow.
...
sales_cohort AS (
  ...
  -- Rolling 90-day window; workflow can override via start_date / end_date env vars
  WHERE ft.dos_date BETWEEN
    DATE_SUB(CURRENT_DATE('Asia/Kolkata'), INTERVAL 90 DAY)
